...
-start the single DRM check with 2sec period and wait for the results
*/
...
void DRMPeriodicCheckStartCallback()
{
	printf("Periodic check started successfully\n");
}
void DRMPeriodicCheckStopCallback()
{
...
int main()
{
	...
	if (fp != 0)
	{
		...
		pidrm_check_periodic_start(DRMPeriodicCheckStartCallback,error_handler, DRMGenunieCallback, DRMFakeCallback, 2, -1);

		sleep(100);
		pidrm_check_periodic_stop(DRMPeriodicCheckStopCallback, error_handler, -1);
	}
	else
