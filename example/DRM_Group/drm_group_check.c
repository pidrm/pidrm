...
This example shows how to perform a single DRM Group check:
-set public key in the API, in this case we read it from a file
-start the single DRM Group check and wait for the result
*/
...
int main()
{
	...
	if (fp != NULL)
	{
		...
		pidrm_group_check(DRMGroupGenunieCallback, DRMGroupFakeCallback, error_handler, -1);
